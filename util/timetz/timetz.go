...
func MakeTimeTZFromLocation(t timeofday.TimeOfDay, loc *time.Location) TimeTZ {
	_, zoneOffsetSecs := timeutil2.Now().In(loc).Zone()
...
func Now() TimeTZ {
	return MakeTimeTZFromTime(timeutil2.Now())
}
...
func (t *TimeTZ) ToTime() time.Time {
	loc := timeutil2.FixedOffsetTimeZoneToLocation(-int(t.OffsetSecs), "TimeTZ")
...
func (t *TimeTZ) ToDuration() time.Duration {
	return t.ToTime().Sub(timeutil2.Unix(0, 0))
}
