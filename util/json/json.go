...
import (
	...
	encoding2 "github.com/iota-uz/psql-parser/util/encoding"
	"github.com/iota-uz/psql-parser/util/unique"
	"math/big"
	"reflect"
	"sort"
	"strconv"
...
func EncodeInvertedIndexKeys(b []byte, json JSON) ([][]byte, error) {
	return json.encodeInvertedIndexKeys(encoding2.EncodeJSONAscending(b))
}
func (j jsonNull) encodeInvertedIndexKeys(b []byte) ([][]byte, error) {
	b = encoding2.AddJSONPathTerminator(b)
	return [][]byte{encoding2.EncodeNullAscending(b)}, nil
}
func (jsonTrue) encodeInvertedIndexKeys(b []byte) ([][]byte, error) {
	b = encoding2.AddJSONPathTerminator(b)
	return [][]byte{encoding2.EncodeTrueAscending(b)}, nil
}
func (jsonFalse) encodeInvertedIndexKeys(b []byte) ([][]byte, error) {
	b = encoding2.AddJSONPathTerminator(b)
	return [][]byte{encoding2.EncodeFalseAscending(b)}, nil
}
func (j jsonString) encodeInvertedIndexKeys(b []byte) ([][]byte, error) {
	b = encoding2.AddJSONPathTerminator(b)
	return [][]byte{encoding2.EncodeStringAscending(b, string(j))}, nil
}
func (j jsonNumber) encodeInvertedIndexKeys(b []byte) ([][]byte, error) {
	b = encoding2.AddJSONPathTerminator(b)
	var dec = apd.Decimal(j)
	return [][]byte{encoding2.EncodeDecimalAscending(b, &dec)}, nil
}
func (j jsonArray) encodeInvertedIndexKeys(b []byte) ([][]byte, error) {
	// Checking for an empty array.
	if len(j) == 0 {
		return [][]byte{encoding2.EncodeJSONEmptyArray(b)}, nil
	}

	prefix := encoding2.EncodeArrayAscending(b)
	var outKeys [][]byte
	for i := range j {
...
func (j jsonObject) encodeInvertedIndexKeys(b []byte) ([][]byte, error) {
	...
	if len(j) == 0 {
		return [][]byte{encoding2.EncodeJSONEmptyObject(b)}, nil
	}
	...
	for i := range j {
		...
		for _, childBytes := range children {
			encodedKey := bytes.Join([][]byte{b,
				encoding2.EncodeJSONKeyStringAscending(nil, string(j[i].k), end),
				childBytes}, nil)
