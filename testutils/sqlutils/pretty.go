...
func VerifyStatementPrettyRoundtrip(t *testing.T, sql string) {
	...
	for i := range stmts {
		...
		prettyFormatted := tree2.AsStringWithFlags(parsedPretty.AST, tree2.FmtSimple)
		origFormatted := tree2.AsStringWithFlags(origStmt, tree2.FmtParsable)
		if prettyFormatted != origFormatted {
			...
			origFormatted = tree2.AsStringWithFlags(reparsedStmt.AST, tree2.FmtParsable)
			if prettyFormatted != origFormatted {
