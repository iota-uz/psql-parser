...
SQLPARSER_TARGETS = \
	sql/parser/sql.go \
	sql/parser/helpmap_test.go \
	sql/parser/help_messages.go \
	sql/lex/tokens.go \
	sql/lex/keywords.go \
	sql/lex/reserved_keywords.go
...
.SECONDARY: sql
sql/parser/gen/sql.go.tmp: sql bin/.bootstrap
	set -euo pipefail; \
	  ret=$$(cd sql/parser/gen && goyacc -p sql -o sql.go.tmp sql-gen.y); \
...
sql/parser/sql.go: sql | bin/.bootstrap
...
.SECONDARY: sql
sql/parser/gen/sql-gen.y: sql sql
	mkdir -p sql/parser/gen
	set -euo pipefail; \
	awk '/func.*sqlSymUnion/ {print $$(NF - 1)}' sql/parser/sql.y | \
	sed -e 's/[]\/$$*.^|[]/\\&/g' | \
	sed -e "s/^/s_(type|token) <(/" | \
	awk '{print $$0")>_\\1 <union> /* <\\2> */_"}' > sql/parser/gen/types_regex.tmp; \
	sed -E -f sql/parser/gen/types_regex.tmp < sql/parser/sql.y | \
	awk -f sql/parser/replace_help_rules.awk | \
	sed -Ee 's,//.*$$,,g;s,/[*]([^*]|[*][^/])*[*]/, ,g;s/ +$$//g' > $@.tmp || rm $@.tmp
	mv -f $@.tmp $@
	rm sql/parser/gen/types_regex.tmp

sql/lex/reserved_keywords.go: sql sql | bin/.bootstrap
	awk -f sql/parser/reserved_keywords.awk < $< > $@.tmp || rm $@.tmp
	mv -f $@.tmp $@
	gofmt -s -w $@

sql/lex/keywords.go: sql sql | bin/.bootstrap
	go run -tags all_keywords sql/lex/all_keywords.go < $< > $@.tmp || rm $@.tmp
	mv -f $@.tmp $@
	gofmt -s -w $@
...
sqlparser-unused-unreserved-keywords: sql sql
	@for kw in $$(awk -f sql/parser/unreserved_keywords.awk < $<); do \
	  if [ $$(grep -c $${kw} $<) -le 2 ]; then \
	    echo $${kw}; \
	  fi \
	done

sql/parser/helpmap_test.go: sql sql | bin/.bootstrap
	@sql/parser/help_gen_test.sh < $< >$@.tmp || rm $@.tmp
	mv -f $@.tmp $@
	gofmt -s -w $@

sql/parser/help_messages.go: sql sql | bin/.bootstrap
	awk -f sql/parser/help.awk < $< > $@.tmp || rm $@.tmp
	mv -f $@.tmp $@
	gofmt -s -w $@
