...
func processCollationOnType(name Name, typ *types2.T, c ColumnCollation) (*types2.T, error) {
	switch typ.Family() {
	case types2.StringFamily:
		return types2.MakeCollatedString(typ, string(c)), nil
	...
	case types2.ArrayFamily:
		...
		return types2.MakeArray(elemTyp), nil
	default:
...
func NewColumnTableDef(
	name Name, typ *types2.T, isSerial bool, qualifications []NamedColumnQualification,
	...
	for _, c := range qualifications {
		...
		case ColumnCollation:
			...
			if err != nil {
				return nil, pgerror2.Wrapf(err, pgcode.Syntax, "invalid locale %s", locale)
			}
		...
		case *ColumnFKConstraint:
			if d.HasFKConstraint() {
				return nil, pgerror2.Newf(pgcode.InvalidTableDefinition,
...
func EvalShardBucketCount(shardBuckets Expr) (int32, error) {
	...
	if !ok {
		return 0, pgerror2.New(pgcode.InvalidParameterValue, invalidBucketCountMsg)
	}
	buckets, err := cst.AsInt32()
	if err != nil || buckets <= 0 {
		if err != nil {
			return 0, pgerror2.Wrap(err, pgcode.InvalidParameterValue, invalidBucketCountMsg)
		}
		return 0, pgerror2.New(pgcode.InvalidParameterValue, invalidBucketCountMsg)
	}
	return buckets, nil
