...
type Constant interface {
	...
	ResolveAsType(*SemaContext, *types2.T) (Datum, error)
}
...
func typeCheckConstant(c Constant, ctx *SemaContext, desired *types2.T) (ret TypedExpr, err error) {
	...
	if desired.Family() == types2.IntFamily {
		if n, ok := c.(*NumVal); ok {
			_, err := n.AsInt64()
			switch err {
...
var errConstNotInt = pgerror2.New(pgcode.NumericValueOutOfRange, "cannot represent numeric constant as an int")
var errConstOutOfRange64 = pgerror2.New(pgcode.NumericValueOutOfRange, "numeric constant out of int64 range")
var errConstOutOfRange32 = pgerror2.New(pgcode.NumericValueOutOfRange, "numeric constant out of int32 range")
...
var (
	intLikeTypes     = []*types2.T{types2.Int, types2.Oid}
	decimalLikeTypes = []*types2.T{types2.Decimal, types2.Float}
...
func (expr *NumVal) AvailableTypes() []*types2.T {
	switch {
...
func (expr *NumVal) DesirableTypes() []*types2.T {
...
func (expr *NumVal) ResolveAsType(ctx *SemaContext, typ *types2.T) (Datum, error) {
	switch typ.Family() {
	...
	case types2.DecimalFamily:
		...
		if idx := strings.IndexRune(s, '/'); idx != -1 {
			...
			if err := dd.SetString(num); err != nil {
				return nil, pgerror2.Wrapf(err, pgcode.Syntax,
	...
	case types2.OidFamily:
		d, err := expr.ResolveAsType(ctx, types2.Int)
		if err != nil {
			return nil, err
		}
		oid := NewDOid(*d.(*DInt))
...
func intersectTypeSlices(xs, ys []*types2.T) (out []*types2.T) {
	for _, x := range xs {
		for _, y := range ys {
			if x == y {
				out = append(out, x)
...
func commonConstantType(vals []Expr, idxs []int) (*types2.T, bool) {
	var candidates []*types2.T
...
var (
	...
	StrValAvailAllParsable = []*types2.T{
	...
	StrValAvailBytes = []*types2.T{types2.Bytes, types2.Uuid, types2.String}
)
...
func (expr *StrVal) AvailableTypes() []*types2.T {
	if expr.scannedAsBytes {
...
// DesirableTypes implements the Constant interface.
func (expr *StrVal) DesirableTypes() []*types2.T {
	return expr.AvailableTypes()
}

// ResolveAsType implements the Constant interface.
func (expr *StrVal) ResolveAsType(ctx *SemaContext, typ *types2.T) (Datum, error) {
	if expr.scannedAsBytes {
