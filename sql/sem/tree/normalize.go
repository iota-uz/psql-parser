...
func (expr *UnaryExpr) normalize(v *NormalizeVisitor) TypedExpr {
	...
	case UnaryMinus:
		...
		if val.ResolvedType().Family() != types2.FloatFamily && v.isNumericZero(val) {
			return val
		}
		switch b := val.(type) {
		// -(a - b) -> (b - a)
...
func (expr *BinaryExpr) normalize(v *NormalizeVisitor) TypedExpr {
	...
	case Minus:
		if types2.IsAdditiveType(left.ResolvedType()) && v.isNumericZero(right) {
			final = ReType(left, expectedType)
			break
		}
	case Mult:
	...
	case EQ, GE, GT, LE, LT:
		...
		for {
			...
			case expr.Operator == EQ && left.Operator == JSONFetchVal && v.isConst(left.Right) &&
				...
				if left.TypedRight().ResolvedType().Family() != types2.StringFamily {
					break
				}
				...
				typedJ, err := dj.TypeCheck(nil, types2.Jsonb)
				if err != nil {
					break
				}
...
func ReType(expr TypedExpr, wantedType *types2.T) TypedExpr {
