...
import (
	...
	"github.com/iota-uz/psql-parser/util/ipaddr"
	"github.com/iota-uz/psql-parser/util/json"
	"github.com/iota-uz/psql-parser/util/stringencoding"
	"github.com/iota-uz/psql-parser/util/timeofday"
	"github.com/iota-uz/psql-parser/util/timetz"
	timeutil2 "github.com/iota-uz/psql-parser/util/timeutil"
	pgdate2 "github.com/iota-uz/psql-parser/util/timeutil/pgdate"
	"github.com/iota-uz/psql-parser/util/uint128"
	uuid2 "github.com/iota-uz/psql-parser/util/uuid"
	"math"
	"math/big"
	"net"
	"regexp"
...
// error string.
func makeParseError(s string, typ *types2.T, err error) error {
	if err != nil {
		return pgerror2.Wrapf(err, pgcode.InvalidTextRepresentation,
			"could not parse %q as type %s", s, typ)
	}
	return pgerror2.Newf(pgcode.InvalidTextRepresentation,
		"could not parse %q as type %s", s, typ)
}
...
func ParseDBool(s string) (*DBool, error) {
	...
	return nil, makeParseError(s, types2.Bool, pgerror2.New(pgcode.InvalidTextRepresentation, "invalid bool value"))
}
...
func ParseDByte(s string) (*DBytes, error) {
	...
	if err != nil {
		return nil, makeParseError(s, types2.Bytes, err)
	}
...
func ParseDUuidFromString(s string) (*DUuid, error) {
	uv, err := uuid2.FromString(s)
	if err != nil {
		return nil, makeParseError(s, types2.Uuid, err)
	}
...
func ParseDUuidFromBytes(b []byte) (*DUuid, error) {
	uv, err := uuid2.FromBytes(b)
	if err != nil {
		return nil, makeParseError(string(b), types2.Uuid, err)
	}
	return NewDUuid(DUuid{uv}), nil
}
...
func GetBool(d Datum) (DBool, error) {
	...
	return false, errors.AssertionFailedf("cannot convert %s to type %s", d.ResolvedType(), types2.Bool)
}

// ResolvedType implements the TypedExpr interface.
func (*DBool) ResolvedType() *types2.T {
	return types2.Bool
}
...
var errCannotCastNegativeIntToBitArray = pgerror2.Newf(pgcode.CannotCoerce,
...
// ResolvedType implements the TypedExpr interface.
func (*DBitArray) ResolvedType() *types2.T {
	return types2.VarBit
}
...
func ParseDInt(s string) (*DInt, error) {
	...
	if err != nil {
		return nil, makeParseError(s, types2.Int, err)
	}
...
// ResolvedType implements the TypedExpr interface.
func (*DInt) ResolvedType() *types2.T {
	return types2.Int
}
...
func ParseDFloat(s string) (*DFloat, error) {
	...
	if err != nil {
		return nil, makeParseError(s, types2.Float, err)
	}
	return NewDFloat(DFloat(f)), nil
}

// ResolvedType implements the TypedExpr interface.
func (*DFloat) ResolvedType() *types2.T {
	return types2.Float
}
...
func (d *DDecimal) SetString(s string) error {
	...
	if res != 0 || err != nil {
		return makeParseError(s, types2.Decimal, nil)
	}
	switch d.Form {
...
// ResolvedType implements the TypedExpr interface.
func (*DDecimal) ResolvedType() *types2.T {
	return types2.Decimal
}
...
// ResolvedType implements the TypedExpr interface.
func (*DString) ResolvedType() *types2.T {
...
func (d *DString) Next(_ *EvalContext) (Datum, bool) {
	return NewDString(string(Key(*d).Next())), true
}
...
// ResolvedType implements the TypedExpr interface.
func (d *DCollatedString) ResolvedType() *types2.T {
	return types2.MakeCollatedString(types2.String, d.Locale)
}
...
// ResolvedType implements the TypedExpr interface.
func (*DBytes) ResolvedType() *types2.T {
	return types2.Bytes
}
...
func (d *DBytes) Next(_ *EvalContext) (Datum, bool) {
	return NewDBytes(DBytes(Key(*d).Next())), true
}
...
// ResolvedType implements the TypedExpr interface.
func (*DUuid) ResolvedType() *types2.T {
	return types2.Uuid
}
...
var DMinUUID = NewDUuid(DUuid{uuid2.UUID{}})

// DMaxUUID is the max UUID.
var DMaxUUID = NewDUuid(DUuid{uuid2.UUID{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
// ResolvedType implements the TypedExpr interface.
func (*DIPAddr) ResolvedType() *types2.T {
	return types2.INet
}
...
func ParseDDate(ctx ParseTimeContext, s string) (*DDate, error) {
	...
	t, err := pgdate2.ParseDate(now, 0 /* mode */, s)
	return NewDDate(t), err
}

// ResolvedType implements the TypedExpr interface.
func (*DDate) ResolvedType() *types2.T {
	return types2.Date
}
...
var (
	epochDate, _ = pgdate2.MakeDateFromPGEpoch(0)
	dEpochDate   = NewDDate(epochDate)
	dMaxDate     = NewDDate(pgdate2.PosInfDate)
	dMinDate     = NewDDate(pgdate2.NegInfDate)
	dLowDate     = NewDDate(pgdate2.LowDate)
	dHighDate    = NewDDate(pgdate2.HighDate)
)

// Prev implements the Datum interface.
func (d *DDate) Prev(_ *EvalContext) (Datum, bool) {
	switch d.Date {
	case pgdate2.PosInfDate:
		return dHighDate, true
	case pgdate2.LowDate:
		return dMinDate, true
	case pgdate2.NegInfDate:
		return nil, false
	}
	n, err := d.AddDays(-1)
	if err != nil {
...
func (d *DDate) IsMax(_ *EvalContext) bool {
	return d.Date == pgdate2.PosInfDate
}

// IsMin implements the Datum interface.
func (d *DDate) IsMin(_ *EvalContext) bool {
	return d.Date == pgdate2.NegInfDate
}
...
func ParseDTime(ctx ParseTimeContext, s string, precision time.Duration) (*DTime, error) {
	...
	s = timeutil2.ReplaceLibPQTimePrefix(s)

	t, err := pgdate2.ParseTime(now, pgdate2.ParseModeYMD, s)
	if err != nil {
		// Build our own error message to avoid exposing the dummy date.
		return nil, makeParseError(s, types2.Time, nil)
	}
	return MakeDTime(timeofday.FromTime(t).Round(precision)), nil
}

// ResolvedType implements the TypedExpr interface.
func (*DTime) ResolvedType() *types2.T {
	return types2.Time
}
...
// ResolvedType implements the TypedExpr interface.
func (*DTimeTZ) ResolvedType() *types2.T {
	return types2.TimeTZ
}
...
func ParseDTimestamp(ctx ParseTimeContext, s string, precision time.Duration) (*DTimestamp, error) {
	now := relativeParseTime(ctx)
	t, err := pgdate2.ParseTimestamp(now, pgdate2.ParseModeYMD, s)
	if err != nil {
		return nil, err
	}
...
// ResolvedType implements the TypedExpr interface.
func (*DTimestamp) ResolvedType() *types2.T {
	return types2.Timestamp
}
...
// ResolvedType implements the TypedExpr interface.
func (*DTimestampTZ) ResolvedType() *types2.T {
	return types2.TimestampTZ
}
...
func NewDInterval(d duration.Duration, itm types2.IntervalTypeMetadata) *DInterval {
...
func ParseDInterval(s string) (*DInterval, error) {
	return ParseDIntervalWithTypeMetadata(s, types2.DefaultIntervalTypeMetadata)
}
...
func truncateDInterval(d *DInterval, itm types2.IntervalTypeMetadata) {
	switch itm.DurationField.DurationType {
	case types2.IntervalDurationType_YEAR:
		d.Duration.Months = d.Duration.Months - d.Duration.Months%12
		d.Duration.Days = 0
		d.Duration.SetNanos(0)
	case types2.IntervalDurationType_MONTH:
		d.Duration.Days = 0
		d.Duration.SetNanos(0)
	case types2.IntervalDurationType_DAY:
		d.Duration.SetNanos(0)
	case types2.IntervalDurationType_HOUR:
		d.Duration.SetNanos(d.Duration.Nanos() - d.Duration.Nanos()%time.Hour.Nanoseconds())
	case types2.IntervalDurationType_MINUTE:
		d.Duration.SetNanos(d.Duration.Nanos() - d.Duration.Nanos()%time.Minute.Nanoseconds())
	case types2.IntervalDurationType_SECOND, types2.IntervalDurationType_UNSET:
...
func ParseDIntervalWithTypeMetadata(s string, itm types2.IntervalTypeMetadata) (*DInterval, error) {
...
func parseDInterval(s string, itm types2.IntervalTypeMetadata) (*DInterval, error) {
	...
	if len(s) == 0 {
		return nil, makeParseError(s, types2.Interval, nil)
	}
	if s[0] == 'P' {
		...
		if err != nil {
			return nil, makeParseError(s, types2.Interval, err)
		}
...
// ResolvedType implements the TypedExpr interface.
func (*DInterval) ResolvedType() *types2.T {
	return types2.Interval
}
...
func ParseDJSON(s string) (Datum, error) {
	...
	if err != nil {
		return nil, pgerror2.Wrapf(err, pgcode.Syntax, "could not parse JSON")
	}
...
// ResolvedType implements the TypedExpr interface.
func (*DJSON) ResolvedType() *types2.T {
	return types2.Jsonb
}
...
func NewDTuple(typ *types2.T, d ...Datum) *DTuple {
	return &DTuple{D: d, typ: typ}
}

// NewDTupleWithLen creates a *DTuple with the provided length.
func NewDTupleWithLen(typ *types2.T, l int) *DTuple {
...
func (d *DTuple) maybePopulateType() {
	if d.typ == nil {
		contents := make([]types2.T, len(d.D))
		for i, v := range d.D {
			contents[i] = *v.ResolvedType()
		}
		d.typ = types2.MakeTuple(contents)
	}
}

// ResolvedType implements the TypedExpr interface.
func (d *DTuple) ResolvedType() *types2.T {
	d.maybePopulateType()
	return d.typ
}
...
func (d *DTuple) Format(ctx *FmtCtx) {
	...
	for i, v := range d.D {
		...
		if parsable && (v == DNull) && len(typ.TupleContents()) > i {
			...
			if typ.TupleContents()[i].Family() != types2.UnknownFamily {
				ctx.WriteString("::")
...
// ResolvedType implements the TypedExpr interface.
func (dNull) ResolvedType() *types2.T {
...
func NewDArray(paramTyp *types2.T) *DArray {
...
// ResolvedType implements the TypedExpr interface.
func (d *DArray) ResolvedType() *types2.T {
	switch d.customOid {
	case oid.T_int2vector:
		return types2.Int2Vector
	case oid.T_oidvector:
		return types2.OidVector
	}
	return types2.MakeArray(d.ParamTyp)
}
...
func (d *DArray) AmbiguousFormat() bool {
	...
	if d.ParamTyp.Family() == types2.UnknownFamily {
...
var errNonHomogeneousArray = pgerror2.New(pgcode.ArraySubscript, "multidimensional arrays must have array expressions with matching dimensions")
...
func (d *DArray) Append(v Datum) error {
	...
	if d.ParamTyp.Family() == types2.ArrayFamily {
		if v == DNull {
...
func MakeDOid(d DInt) DOid {
	return DOid{DInt: d, semanticType: types2.Oid, name: ""}
}
...
// and a string.
func NewDOidWithName(d DInt, typ *types2.T, name string) *DOid {
	return &DOid{
		DInt:         d,
		semanticType: typ,
		name:         name,
...
func (d *DOid) ResolvedType() *types2.T {
...
// ResolvedType implements the TypedExpr interface.
func (d *DOidWrapper) ResolvedType() *types2.T {
	return types2.OidToType[d.Oid]
}
...
func NewDefaultDatum(evalCtx *EvalContext, t *types2.T) (d Datum, err error) {
	switch t.Family() {
...
func DatumTypeSize(t *types2.T) (size uintptr, isVarlen bool) {
	// The following are composite types or types that support multiple widths.
	switch t.Family() {
	case types2.TupleFamily:
		if types2.IsWildcardTupleType(t) {
		...
		return sz, variable
	case types2.IntFamily, types2.FloatFamily:
...
var baseDatumTypeSizes = map[types2.Family]struct {
	sz       uintptr
	variable bool
}{
	types2.UnknownFamily:        {unsafe.Sizeof(dNull{}), fixedSize},
	types2.BoolFamily:           {unsafe.Sizeof(DBool(false)), fixedSize},
	types2.BitFamily:            {unsafe.Sizeof(DBitArray{}), variableSize},
	types2.IntFamily:            {unsafe.Sizeof(DInt(0)), fixedSize},
	types2.FloatFamily:          {unsafe.Sizeof(DFloat(0.0)), fixedSize},
	types2.DecimalFamily:        {unsafe.Sizeof(DDecimal{}), variableSize},
	types2.StringFamily:         {unsafe.Sizeof(DString("")), variableSize},
	types2.CollatedStringFamily: {unsafe.Sizeof(DCollatedString{"", "", nil}), variableSize},
	types2.BytesFamily:          {unsafe.Sizeof(DBytes("")), variableSize},
	types2.DateFamily:           {unsafe.Sizeof(DDate{}), fixedSize},
	types2.TimeFamily:           {unsafe.Sizeof(DTime(0)), fixedSize},
	types2.TimeTZFamily:         {unsafe.Sizeof(DTimeTZ{}), fixedSize},
	types2.TimestampFamily:      {unsafe.Sizeof(DTimestamp{}), fixedSize},
	types2.TimestampTZFamily:    {unsafe.Sizeof(DTimestampTZ{}), fixedSize},
	types2.IntervalFamily:       {unsafe.Sizeof(DInterval{}), fixedSize},
	types2.JsonFamily:           {unsafe.Sizeof(DJSON{}), variableSize},
	types2.UuidFamily:           {unsafe.Sizeof(DUuid{}), fixedSize},
	types2.INetFamily:           {unsafe.Sizeof(DIPAddr{}), fixedSize},
	types2.OidFamily:            {unsafe.Sizeof(DInt(0)), fixedSize},

	// TODO(jordan,justin): This seems suspicious.
	types2.ArrayFamily: {unsafe.Sizeof(DString("")), variableSize},

	// TODO(jordan,justin): This seems suspicious.
	types2.AnyFamily: {unsafe.Sizeof(DString("")), variableSize},
}
