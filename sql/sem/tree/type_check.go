...
import (
	...
	"github.com/iota-uz/psql-parser/sql/pgwire/pgcode"
	pgerror2 "github.com/iota-uz/psql-parser/sql/pgwire/pgerror"
	"github.com/iota-uz/psql-parser/sql/sessiondata"
	types2 "github.com/iota-uz/psql-parser/sql/types"
	"github.com/iota-uz/psql-parser/util/errorutil/unimplemented"
	"github.com/iota-uz/psql-parser/util/hlc"
...
func placeholderTypeAmbiguityError(idx PlaceholderIdx) error {
	return pgerror2.WithCandidateCode(
...
func unexpectedTypeError(expr Expr, want, got *types2.T) error {
	return pgerror2.Newf(pgcode.InvalidParameterValue,
...
func decorateTypeCheckError(err error, format string, a ...interface{}) error {
	...
	return pgerror2.Wrapf(err, pgcode.InvalidParameterValue, format, a...)
}
...
// desired.
func TypeCheck(expr Expr, ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	if desired == nil {
...
func TypeCheckAndRequire(
	expr Expr, ctx *SemaContext, required *types2.T, op string,
) (TypedExpr, error) {
	typedExpr, err := TypeCheck(expr, ctx, required)
	if err != nil {
		return nil, err
	}
	if typ := typedExpr.ResolvedType(); !(typ.Equivalent(required) || typ.Family() == types2.UnknownFamily) {
		return typedExpr, pgerror2.Newf(
...
func (expr *AndExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	...
	expr.typ = types2.Bool
	return expr, nil
}

// TypeCheck implements the Expr interface.
func (expr *BinaryExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	...
	if leftReturn.Family() == types2.UnknownFamily || rightReturn.Family() == types2.UnknownFamily {
		if len(fns) > 0 {
			noneAcceptNull := true
			for _, e := range fns {
	...
	if len(fns) != 1 {
		var desStr string
		if desired.Family() != types2.AnyFamily {
			desStr = fmt.Sprintf(" (desired <%s>)", desired)
		}
		sig := fmt.Sprintf("<%s> %s <%s>%s", leftReturn, expr.Operator, rightReturn, desStr)
		if len(fns) == 0 {
			return nil,
				pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedBinaryOpErrFmt, sig)
		}
		fnsStr := formatCandidates(expr.Operator.String(), fns)
		err = pgerror2.Newf(pgcode.AmbiguousFunction, ambiguousBinaryOpErrFmt, sig)
...
// TypeCheck implements the Expr interface.
func (expr *CaseExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	var err error
	...
	if expr.Expr != nil {
		...
		typedSubExprs, _, err := TypeCheckSameTypedExprs(ctx, types2.Any, tmpExprs...)
		if err != nil {
...
func isCastDeepValid(castFrom, castTo *types2.T) bool {
	if castTo.Family() == types2.ArrayFamily && castFrom.Family() == types2.ArrayFamily {
...
// TypeCheck implements the Expr interface.
func (expr *CastExpr) TypeCheck(ctx *SemaContext, _ *types2.T) (TypedExpr, error) {
	...
	case isConstant(expr.Expr):
		if canConstantBecome(expr.Expr.(Constant), expr.Type) {
			...
			case types2.BoolFamily, types2.DateFamily, types2.TimeFamily, types2.TimestampFamily, types2.TimestampTZFamily,
				types2.IntervalFamily, types2.BytesFamily:
	...
	case isEmptyArray(expr.Expr):
		...
		// the desired type.
		if expr.Type.Family() == types2.ArrayFamily {
			desired = expr.Type
		}
	}
	...
	return nil, pgerror2.Newf(pgcode.CannotCoerce, "invalid cast: %s -> %s", castFrom, expr.Type)
}

// TypeCheck implements the Expr interface.
func (expr *IndirectionExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	for i, t := range expr.Indirection {
		...
		beginExpr, err := typeCheckAndRequire(ctx, t.Begin, types2.Int, "ARRAY subscript")
		if err != nil {
			return nil, err
		}
		t.Begin = beginExpr
	...
	subExpr, err := expr.Expr.TypeCheck(ctx, types2.MakeArray(desired))
	if err != nil {
		return nil, err
	}
	typ := subExpr.ResolvedType()
	if typ.Family() != types2.ArrayFamily {
		return nil, pgerror2.Newf(pgcode.DatatypeMismatch, "cannot subscript type %s because it is not an array", typ)
	}
	expr.Expr = subExpr
...
// TypeCheck implements the Expr interface.
func (expr *AnnotateTypeExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// TypeCheck implements the Expr interface.
func (expr *CollateExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	_, err := language.Parse(expr.Locale)
	if err != nil {
		return nil, pgerror2.Wrapf(err, pgcode.InvalidParameterValue,
			"invalid locale %s", expr.Locale)
	}
	subExpr, err := expr.Expr.TypeCheck(ctx, types2.String)
	if err != nil {
		return nil, err
	}
	t := subExpr.ResolvedType()
	if types2.IsStringType(t) || t.Family() == types2.UnknownFamily {
		expr.Expr = subExpr
		expr.typ = types2.MakeCollatedString(types2.String, expr.Locale)
		return expr, nil
	}
	return nil, pgerror2.Newf(pgcode.DatatypeMismatch,
...
// type.
func NewTypeIsNotCompositeError(resolvedType *types2.T) error {
	return pgerror2.Newf(pgcode.WrongObjectType,
		"type %s is not composite", resolvedType,
	)
}

// TypeCheck implements the Expr interface.
func (expr *TupleStar) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	...
	if resolvedType.Family() != types2.TupleFamily {
...
func (expr *TupleStar) ResolvedType() *types2.T {
	return expr.Expr.(TypedExpr).ResolvedType()
}

// TypeCheck implements the Expr interface.
func (expr *ColumnAccessExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	...
	subExpr, err := expr.Expr.TypeCheck(ctx, types2.Any)
	if err != nil {
		return nil, err
	}
	...
	if resolvedType.Family() != types2.TupleFamily || (!expr.ByIndex && len(resolvedType.TupleLabels()) == 0) {
		return nil, NewTypeIsNotCompositeError(resolvedType)
	}
	...
	if expr.ByIndex {
		...
		if expr.ColIndex < 0 || expr.ColIndex >= len(resolvedType.TupleContents()) {
			return nil, pgerror2.Newf(pgcode.Syntax, "tuple column %d does not exist", expr.ColIndex+1)
		}
	} else {
		...
		for i, label := range resolvedType.TupleLabels() {
			if label == expr.ColName {
				if expr.ColIndex != -1 {
					...
					return nil, pgerror2.Newf(pgcode.AmbiguousColumn, "column reference %q is ambiguous", label)
				}
				expr.ColIndex = i
			}
		}
...
// TypeCheck implements the Expr interface.
func (expr *CoalesceExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// TypeCheck implements the Expr interface.
func (expr *ComparisonExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
var (
	errStarNotAllowed      = pgerror2.New(pgcode.Syntax, "cannot use \"*\" in this context")
	errInvalidDefaultUsage = pgerror2.New(pgcode.Syntax, "DEFAULT can only appear in a VALUES list within INSERT or on the right side of a SET")
	errInvalidMaxUsage     = pgerror2.New(pgcode.Syntax, "MAXVALUE can only appear within a range partition expression")
	errInvalidMinUsage     = pgerror2.New(pgcode.Syntax, "MINVALUE can only appear within a range partition expression")
	errPrivateFunction     = pgerror2.New(pgcode.ReservedName, "function reserved for internal use")
)
...
func NewAggInAggError() error {
	return pgerror2.Newf(pgcode.Grouping, "aggregate function calls cannot be nested")
}

// NewInvalidNestedSRFError creates a rejection for a nested SRF.
func NewInvalidNestedSRFError(context string) error {
	return pgerror2.Newf(pgcode.FeatureNotSupported,
...
func NewInvalidFunctionUsageError(class FunctionClass, context string) error {
	...
	return pgerror2.Newf(code, "%s functions are not allowed in %s", cat, context)
}
...
func (sc *SemaContext) checkFunctionUsage(expr *FuncExpr, def *FunctionDefinition) error {
	...
	if def.Private {
		return pgerror2.Wrapf(errPrivateFunction, pgcode.ReservedName,
	...
	if expr.IsWindowFunctionApplication() {
		...
		if sc.Properties.Derived.InWindowFunc &&
			...
			return pgerror2.Newf(pgcode.Windowing, "window function calls cannot be nested")
		}
...
// TypeCheck implements the Expr interface.
func (expr *FuncExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	...
	if !def.NullableArgs && def.FunctionProperties.Class == AggregateClass {
		for i := range typedSubExprs {
			if typedSubExprs[i].ResolvedType().Family() == types2.UnknownFamily {
				var filtered []overloadImpl
				for j := range fns {
					if fns[j].params().GetAt(i).Equivalent(types2.String) {
						if filtered == nil {
				...
				if filtered != nil {
					...
					e, err := NewTypedCastExpr(typedSubExprs[i], types2.String)
					if err != nil {
						return nil, err
					}
					typedSubExprs[i] = e
	...
	if !def.NullableArgs && def.FunctionProperties.Class != GeneratorClass &&
		...
		for _, expr := range typedSubExprs {
			if expr.ResolvedType().Family() == types2.UnknownFamily {
				return DNull, nil
			}
		}
	}
	...
	if len(fns) != 1 {
		...
		if len(fns) == 0 {
			return nil, pgerror2.Newf(pgcode.UndefinedFunction, "unknown signature: %s", sig)
		}
		fnsStr := formatCandidates(expr.Func.String(), fns)
		return nil, pgerror2.Newf(pgcode.AmbiguousFunction, "ambiguous call: %s, candidates are:\n%s", sig, fnsStr)
	}
	...
	if expr.IsWindowFunctionApplication() {
		...
		if expr.Type == DistinctFuncType {
			return nil, pgerror2.New(pgcode.FeatureNotSupported, "DISTINCT is not implemented for window functions")
		}
	} else {
		// Make sure the window function builtins are used as window function applications.
		if def.Class == WindowClass {
			return nil, pgerror2.Newf(pgcode.WrongObjectType,
	...
	if expr.OrderBy != nil {
		for i := range expr.OrderBy {
			typedExpr, err := expr.OrderBy[i].Expr.TypeCheck(ctx, types2.Any)
			if err != nil {
				return nil, err
			}
...
// TypeCheck implements the Expr interface.
func (expr *IfErrExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	var typedCond, typedElse TypedExpr
	var retType *types2.T
	var err error
	if expr.Else == nil {
		typedCond, err = expr.Cond.TypeCheck(ctx, types2.Any)
		if err != nil {
			return nil, err
		}
		retType = types2.Bool
	...
	if expr.ErrCode != nil {
		typedErrCode, err = typeCheckAndRequire(ctx, expr.ErrCode, types2.String, "IFERROR")
		if err != nil {
			return nil, err
		}
	}
...
// TypeCheck implements the Expr interface.
func (expr *IfExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// TypeCheck implements the Expr interface.
func (expr *IsOfTypeExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	exprTyped, err := expr.Expr.TypeCheck(ctx, types2.Any)
	if err != nil {
		return nil, err
	}
	expr.Expr = exprTyped
...
// TypeCheck implements the Expr interface.
func (expr *NotExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// TypeCheck implements the Expr interface.
func (expr *NullIfExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// TypeCheck implements the Expr interface.
func (expr *OrExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// TypeCheck implements the Expr interface.
func (expr *ParenExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// checking.
func (expr *ColumnItem) TypeCheck(_ *SemaContext, desired *types2.T) (TypedExpr, error) {
	name := expr.String()
	if _, ok := presetTypesForTesting[name]; ok {
		return expr, nil
	}
	return nil, pgerror2.Newf(pgcode.UndefinedColumn,
		"column %q does not exist", ErrString(expr))
}

// TypeCheck implements the Expr interface.
func (expr UnqualifiedStar) TypeCheck(_ *SemaContext, desired *types2.T) (TypedExpr, error) {
	return nil, errStarNotAllowed
}

// TypeCheck implements the Expr interface.
func (expr *UnresolvedName) TypeCheck(s *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// TypeCheck implements the Expr interface.
func (expr *AllColumnsSelector) TypeCheck(_ *SemaContext, desired *types2.T) (TypedExpr, error) {
	return nil, pgerror2.Newf(pgcode.Syntax, "cannot use %q in this context", expr)
}

// TypeCheck implements the Expr interface.
func (expr *RangeCond) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
// TypeCheck implements the Expr interface.
func (expr *Subquery) TypeCheck(sc *SemaContext, _ *types2.T) (TypedExpr, error) {
	if sc != nil && sc.Properties.required.rejectFlags&RejectSubqueries != 0 {
		return nil, pgerror2.Newf(pgcode.FeatureNotSupported,
...
// TypeCheck implements the Expr interface.
func (expr *UnaryExpr) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	...
	if len(fns) > 0 {
		if exprReturn.Family() == types2.UnknownFamily {
			return DNull, nil
		}
	}
	...
	if len(fns) != 1 {
		...
		if len(fns) == 0 {
			return nil,
				pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedUnaryOpErrFmt, sig)
		}
		fnsStr := formatCandidates(expr.Operator.String(), fns)
		err = pgerror2.Newf(pgcode.AmbiguousFunction, ambiguousUnaryOpErrFmt, sig)
		err = errors.WithHintf(err, candidatesHintFmt, fnsStr)
		return nil, err
	}
...
// TypeCheck implements the Expr interface.
func (expr DefaultVal) TypeCheck(_ *SemaContext, desired *types2.T) (TypedExpr, error) {
	return nil, errInvalidDefaultUsage
}

// TypeCheck implements the Expr interface.
func (expr PartitionMinVal) TypeCheck(_ *SemaContext, desired *types2.T) (TypedExpr, error) {
	return nil, errInvalidMinUsage
}

// TypeCheck implements the Expr interface.
func (expr PartitionMaxVal) TypeCheck(_ *SemaContext, desired *types2.T) (TypedExpr, error) {
	return nil, errInvalidMaxUsage
}

// TypeCheck implements the Expr interface.
func (expr *NumVal) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	return typeCheckConstant(expr, ctx, desired)
}

// TypeCheck implements the Expr interface.
func (expr *StrVal) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	return typeCheckConstant(expr, ctx, desired)
}

// TypeCheck implements the Expr interface.
func (expr *Tuple) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	// Ensure the number of labels matches the number of expressions.
	if len(expr.Labels) > 0 && len(expr.Labels) != len(expr.Exprs) {
		return nil, pgerror2.Newf(pgcode.Syntax,
	...
	var labels []string
	contents := make([]types2.T, len(expr.Exprs))
	for i, subExpr := range expr.Exprs {
		desiredElem := types2.Any
		if desired.Family() == types2.TupleFamily && len(desired.TupleContents()) > i {
	...
	expr.typ = types2.MakeLabeledTuple(contents, labels)
	return expr, nil
}

var errAmbiguousArrayType = pgerror2.Newf(pgcode.IndeterminateDatatype, "cannot determine type of empty array. "+
	"Consider annotating with the desired type, for example ARRAY[]:::int[]")

// TypeCheck implements the Expr interface.
func (expr *Array) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	desiredParam := types2.Any
	if desired.Family() == types2.ArrayFamily {
		desiredParam = desired.ArrayContents()
	}

	if len(expr.Exprs) == 0 {
		if desiredParam.Family() == types2.AnyFamily {
			return nil, errAmbiguousArrayType
		}
		expr.typ = types2.MakeArray(desiredParam)
		return expr, nil
	}
...
// TypeCheck implements the Expr interface.
func (expr *ArrayFlatten) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
	desiredParam := types2.Any
	if desired.Family() == types2.ArrayFamily {
		desiredParam = desired.ArrayContents()
	}
	...
	expr.typ = types2.MakeArray(subqueryTyped.ResolvedType())
...
// TypeCheck implements the Expr interface.
func (expr *Placeholder) TypeCheck(ctx *SemaContext, desired *types2.T) (TypedExpr, error) {
...
func (d *DBitArray) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DBool) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DInt) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DFloat) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DDecimal) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DString) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DCollatedString) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DBytes) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DUuid) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DIPAddr) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DDate) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DTime) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DTimeTZ) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DTimestamp) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DTimestampTZ) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DInterval) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DJSON) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DTuple) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DArray) TypeCheck(_ *SemaContext, desired *types2.T) (TypedExpr, error) {
	...
	// ARRAY[]
	// ARRAY[NULL, NULL]
	if (d.ParamTyp == types2.Unknown || d.ParamTyp == types2.Any) && (!d.HasNonNulls) {
		if desired.Family() != types2.ArrayFamily {
...
// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DOid) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d *DOidWrapper) TypeCheck(_ *SemaContext, _ *types2.T) (TypedExpr, error) { return d, nil }

// TypeCheck implements the Expr interface. It is implemented as an idempotent
// identity function for Datum.
func (d dNull) TypeCheck(_ *SemaContext, desired *types2.T) (TypedExpr, error) { return d, nil }
...
func typeCheckAndRequireTupleElems(
	...
	tuple.typ = types2.MakeTuple(make([]types2.T, len(tuple.Exprs)))
...
func typeCheckAndRequireBoolean(ctx *SemaContext, expr Expr, op string) (TypedExpr, error) {
	return typeCheckAndRequire(ctx, expr, types2.Bool, op)
}

func typeCheckAndRequire(
	ctx *SemaContext, expr Expr, required *types2.T, op string,
) (TypedExpr, error) {
	typedExpr, err := expr.TypeCheck(ctx, required)
	if err != nil {
		return nil, err
	}
	if typ := typedExpr.ResolvedType(); !(typ.Family() == types2.UnknownFamily || typ.Equivalent(required)) {
		return nil, pgerror2.Newf(pgcode.DatatypeMismatch, "incompatible %s type: %s", op, typ)
	}
	return typedExpr, nil
}
...
func typeCheckComparisonOpWithSubOperator(
	...
	var cmpTypeLeft, cmpTypeRight *types2.T
	var leftTyped, rightTyped TypedExpr
	if array, isConstructor := right.(*Array); isConstructor {
		...
		typedSubExprs, retType, err := TypeCheckSameTypedExprs(ctx, types2.Any, sameTypeExprs...)
		if err != nil {
			sigWithErr := fmt.Sprintf(compExprsWithSubOpFmt, left, subOp, op, right, err)
			return nil, nil, nil, false,
				pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedCompErrFmt, sigWithErr)
		}
		...
		array.typ = types2.MakeArray(retType)

		rightTyped = array
		cmpTypeRight = retType

		// Return early without looking up a CmpOp if the comparison type is types.Null.
		if leftTyped.ResolvedType().Family() == types2.UnknownFamily || retType.Family() == types2.UnknownFamily {
		...
		var err error
		leftTyped, err = left.TypeCheck(ctx, types2.Any)
		if err != nil {
		...
		if tuple, ok := right.(*Tuple); ok {
			...
			rightTyped, err = right.TypeCheck(ctx, types2.MakeArray(cmpTypeLeft))
			if err != nil {
				return nil, nil, nil, false, err
			}
		}
		...
		rightReturn := rightTyped.ResolvedType()
		if cmpTypeLeft.Family() == types2.UnknownFamily || rightReturn.Family() == types2.UnknownFamily {
			return leftTyped, rightTyped, nil, true /* alwaysNull */, nil
		}
		...
		default:
			...
			return nil, nil, nil, false, pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedCompErrFmt, sigWithErr)
		}
	}
...
func subOpCompError(leftType, rightType *types2.T, subOp, op ComparisonOperator) error {
	sig := fmt.Sprintf(compSignatureWithSubOpFmt, leftType, subOp, op, rightType)
	return pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedCompErrFmt, sig)
}

// typeCheckSubqueryWithIn checks the case where the right side of an IN
// expression is a subquery.
func typeCheckSubqueryWithIn(left, right *types2.T) error {
	if right.Family() == types2.TupleFamily {
...
func typeCheckComparisonOp(
	...
	case foldedOp == In && rightIsTuple:
		...
		fn, ok := ops.LookupImpl(retType, types2.AnyTuple)
		if !ok {
			sig := fmt.Sprintf(compSignatureFmt, retType, op, types2.AnyTuple)
			return nil, nil, nil, false,
				pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedCompErrFmt, sig)
		}

		typedLeft := typedSubExprs[0]
		typedSubExprs = typedSubExprs[1:]

		rightTuple.typ = types2.MakeTuple(make([]types2.T, len(typedSubExprs)))
	...
	case foldedOp == In && rightIsSubquery:
		typedLeft, err := foldedLeft.TypeCheck(ctx, types2.Any)
		if err != nil {
			sigWithErr := fmt.Sprintf(compExprsFmt, left, op, right, err)
			return nil, nil, nil, false,
				pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedCompErrFmt, sigWithErr)
		...
		fn, ok := ops.LookupImpl(typ, types2.AnyTuple)
		if !ok {
			sig := fmt.Sprintf(compSignatureFmt, typ, op, types2.AnyTuple)
			return nil, nil, nil, false,
				pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedCompErrFmt, sig)
		}

		desired := types2.MakeTuple([]types2.T{*typ})
	...
	case leftIsTuple && rightIsTuple:
		fn, ok := ops.LookupImpl(types2.AnyTuple, types2.AnyTuple)
		if !ok {
			sig := fmt.Sprintf(compSignatureFmt, types2.AnyTuple, op, types2.AnyTuple)
			return nil, nil, nil, false,
				pgerror2.Newf(pgcode.InvalidParameterValue, unsupportedCompErrFmt, sig)
		}
	...
	typedSubExprs, fns, err := typeCheckOverloadedExprs(
		ctx, types2.Any, ops, true /* inBinOp */, foldedLeft, foldedRight,
	)
	if err != nil {
		return nil, nil, nil, false, err
	}
	...
	if leftFamily == types2.UnknownFamily || rightFamily == types2.UnknownFamily {
		nullComparison = true
		if len(fns) > 0 {
			noneAcceptNull := true
			for _, e := range fns {
	...
	leftIsGeneric := leftFamily == types2.CollatedStringFamily || leftFamily == types2.ArrayFamily
	rightIsGeneric := rightFamily == types2.CollatedStringFamily || rightFamily == types2.ArrayFamily
	...
	if len(fns) != 1 || typeMismatch {
		...
		err = pgerror2.Newf(pgcode.AmbiguousFunction, ambiguousCompErrFmt, sig)
		err = errors.WithHintf(err, candidatesHintFmt, fnsStr)
		return nil, nil, nil, false, err
	}
...
func TypeCheckSameTypedExprs(
	ctx *SemaContext, desired *types2.T, exprs ...Expr,
) ([]TypedExpr, *types2.T, error) {
	switch len(exprs) {
	case 0:
		return nil, nil, nil
	case 1:
		...
		if typ == types2.Unknown && desired != types2.Any {
	...
	default:
		...
		for i, j := range resolvableIdxs {
			...
			if returnType := typedExpr.ResolvedType(); returnType.Family() != types2.UnknownFamily {
		...
		if firstValidType.Family() == types2.UnknownFamily {
			...
			default:
				if desired != types2.Any {
					return typedExprs, desired, nil
				}
				return typedExprs, types2.Unknown, nil
			}
		}
		...
		for _, i := range resolvableIdxs[firstValidIdx+1:] {
			...
			if typ := typedExpr.ResolvedType(); !(typ.Equivalent(firstValidType) || typ.Family() == types2.UnknownFamily) {
...
func typeCheckSameTypedPlaceholders(s typeCheckExprsState, typ *types2.T) error {
...
func typeCheckSameTypedConsts(
	s typeCheckExprsState, typ *types2.T, required bool,
) (*types2.T, error) {
	setTypeForConsts := func(typ *types2.T) (*types2.T, error) {
	...
	if typ.Family() != types2.AnyFamily {
		all := true
		for _, i := range s.constIdxs {
			if !canConstantBecome(s.exprs[i].(Constant), typ) {
				if required {
					typedExpr, err := s.exprs[i].TypeCheck(s.ctx, types2.Any)
					if err != nil {
						return nil, err
					}
	...
	for _, i := range s.constIdxs {
		...
		if reqTyp.Family() == types2.AnyFamily {
...
func typeCheckConstsAndPlaceholdersWithDesired(
	s typeCheckExprsState, desired *types2.T,
) ([]TypedExpr, *types2.T, error) {
...
func typeCheckTupleComparison(
	...
	left.typ = types2.MakeTuple(make([]types2.T, tupLen))
	right.typ = types2.MakeTuple(make([]types2.T, tupLen))
	for elemIdx := range left.Exprs {
		...
		if err != nil {
			...
			return nil, nil, pgerror2.Newf(pgcode.DatatypeMismatch, "tuples %s are not comparable at index %d: %s",
				&exps, elemIdx+1, err)
		}
...
func typeCheckSameTypedTupleExprs(
	...
	resTypes := types2.MakeTuple(make([]types2.T, firstLen))
	...
	for elemIdx := range first.Exprs {
		...
		if err != nil {
			return nil, nil, pgerror2.Newf(pgcode.DatatypeMismatch, "tuples %s are not the same type: %v", Exprs(exprs), err)
		}
...
func checkAllExprsAreTuplesOrNulls(ctx *SemaContext, exprs []Expr) error {
	for _, expr := range exprs {
		...
		isNull := expr == DNull
		if !(isTuple || isNull) {
			typedExpr, err := expr.TypeCheck(ctx, types2.Any)
			if err != nil {
				return err
			}
			return unexpectedTypeError(expr, types2.AnyTuple, typedExpr.ResolvedType())
		}
	}
	return nil
}
...
func checkTupleHasLength(t *Tuple, expectedLen int) error {
	if len(t.Exprs) != expectedLen {
		return pgerror2.Newf(pgcode.DatatypeMismatch, "expected tuple %v to have a length of %d", t, expectedLen)
	}
	return nil
}
		...
		if arg, ok := t.Expr.(*Placeholder); ok {
			...
			case typeFromAnnotation:
				...
				if !t.Type.Equivalent(v.types[arg.Idx]) {
					v.setErr(arg.Idx, pgerror2.Newf(
						pgcode.DatatypeMismatch,
