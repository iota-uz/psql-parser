...
type Overload struct {
	...
	AggregateFunc func([]*types2.T, *EvalContext, Datums) AggregateFunc
	WindowFunc    func([]*types2.T, *EvalContext) WindowFunc
...
func (b Overload) FixedReturnType() *types2.T {
	if b.ReturnType == nil {
		return nil
	}
...
func (b Overload) InferReturnTypeFromInputArgTypes(inputTypes []*types2.T) *types2.T {
...
func (b Overload) Signature(simplify bool) string {
	...
	if simplify {
		if retType.Family() == types2.TupleFamily && len(retType.TupleContents()) == 1 {
...
type TypeList interface {
	...
	MatchAt(typ *types2.T, i int) bool
...
func (a ArgTypes) Match(types []*types2.T) bool {
	if len(types) != len(a) {
		return false
	}
	for i := range types {
...
func (a ArgTypes) MatchAt(typ *types2.T, i int) bool {
	...
	if typ.Family() == types2.TupleFamily {
		typ = types2.AnyTuple
	}
	return i < len(a) && (typ.Family() == types2.UnknownFamily || a[i].Typ.Equivalent(typ))
}
...
func (a ArgTypes) GetAt(i int) *types2.T {
	return a[i].Typ
}
...
func (a ArgTypes) Types() []*types2.T {
	n := len(a)
	ret := make([]*types2.T, n)
	for i, s := range a {
		ret[i] = s.Typ
...
// Match is part of the TypeList interface.
func (HomogeneousType) Match(types []*types2.T) bool {
	return true
}

// MatchAt is part of the TypeList interface.
func (HomogeneousType) MatchAt(typ *types2.T, i int) bool {
	return true
}
...
// GetAt is part of the TypeList interface.
func (HomogeneousType) GetAt(i int) *types2.T {
	return types2.Any
}
...
// Types is part of the TypeList interface.
func (HomogeneousType) Types() []*types2.T {
	return []*types2.T{types2.Any}
}
...
// Match is part of the TypeList interface.
func (v VariadicType) Match(types []*types2.T) bool {
	for i := range types {
...
// MatchAt is part of the TypeList interface.
func (v VariadicType) MatchAt(typ *types2.T, i int) bool {
	if i < len(v.FixedTypes) {
		return typ.Family() == types2.UnknownFamily || v.FixedTypes[i].Equivalent(typ)
	}
	return typ.Family() == types2.UnknownFamily || v.VarType.Equivalent(typ)
}
...
// GetAt is part of the TypeList interface.
func (v VariadicType) GetAt(i int) *types2.T {
	if i < len(v.FixedTypes) {
...
// Types is part of the TypeList interface.
func (v VariadicType) Types() []*types2.T {
	result := make([]*types2.T, len(v.FixedTypes)+1)
...
var UnknownReturnType *types2.T
...
func FirstNonNullReturnType() ReturnTyper {
	return func(args []TypedExpr) *types2.T {
		...
		for _, arg := range args {
			if t := arg.ResolvedType(); t.Family() != types2.UnknownFamily {
				return t
			}
		}
...
func returnTypeToFixedType(s ReturnTyper, inputTyps []TypedExpr) *types2.T {
...
func typeCheckOverloadedExprs(
	ctx *SemaContext, desired *types2.T, overloads []overloadImpl, inBinOp bool, exprs ...Expr,
	...
	if len(overloads) == 0 {
		for _, i := range s.resolvableIdxs {
			typ, err := exprs[i].TypeCheck(ctx, types2.Any)
			if err != nil {
	...
	if inBinOp && len(s.exprs) == 2 {
		if ok, typedExprs, fns, err := filterAttempt(ctx, &s, func() {
			...
			left := s.typedExprs[0]
			if left == nil {
				left, err = s.exprs[0].TypeCheck(ctx, types2.Any)
				if err != nil {
					return
				}
			}
			...
			if right == nil {
				right, err = s.exprs[1].TypeCheck(ctx, types2.Any)
				if err != nil {
					return
				}
			}
			...
			leftIsNull := leftType.Family() == types2.UnknownFamily
			rightIsNull := rightType.Family() == types2.UnknownFamily
...
func defaultTypeCheck(ctx *SemaContext, s *typeCheckOverloadState, errorOnPlaceholders bool) error {
	for _, i := range s.constIdxs {
		typ, err := s.exprs[i].TypeCheck(ctx, types2.Any)
		if err != nil {
	...
	for _, i := range s.placeholderIdxs {
		if errorOnPlaceholders {
			_, err := s.exprs[i].TypeCheck(ctx, types2.Any)
			return err
		}
