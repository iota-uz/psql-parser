...
import (
	...
	"github.com/iota-uz/psql-parser/util/arith"
	"github.com/iota-uz/psql-parser/util/bitarray"
	"github.com/iota-uz/psql-parser/util/duration"
	"github.com/iota-uz/psql-parser/util/hlc"
	json2 "github.com/iota-uz/psql-parser/util/json"
	"github.com/iota-uz/psql-parser/util/timeofday"
	timeutil2 "github.com/iota-uz/psql-parser/util/timeutil"
...
var (
	...
	ErrIntOutOfRange = pgerror2.New(pgcode.NumericValueOutOfRange, "integer out of range")
	// ErrFloatOutOfRange is reported when float arithmetic overflows.
	ErrFloatOutOfRange = pgerror2.New(pgcode.NumericValueOutOfRange, "float out of range")
	errDecOutOfRange   = pgerror2.New(pgcode.NumericValueOutOfRange, "decimal out of range")

	// ErrDivByZero is reported on a division by zero.
	ErrDivByZero = pgerror2.New(pgcode.DivisionByZero, "division by zero")
	// ErrZeroModulus is reported when computing the rest of a division by zero.
	ErrZeroModulus = pgerror2.New(pgcode.DivisionByZero, "zero modulus")
...
func (op *BinOp) matchParams(l, r *types2.T) bool {
...
func AppendToMaybeNullArray(typ *types2.T, left Datum, right Datum) (Datum, error) {
...
func PrependToMaybeNullArray(typ *types2.T, left Datum, right Datum) (Datum, error) {
	result := NewDArray(typ)
...
func initArrayElementConcatenation() {
	for _, t := range types2.Scalar {
		typ := t
		BinOps[Concat] = append(BinOps[Concat], &BinOp{
			LeftType:     types2.MakeArray(typ),
			RightType:    typ,
			ReturnType:   types2.MakeArray(typ),
			NullableArgs: true,
		...
		BinOps[Concat] = append(BinOps[Concat], &BinOp{
			LeftType:     typ,
			RightType:    types2.MakeArray(typ),
			ReturnType:   types2.MakeArray(typ),
			NullableArgs: true,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
...
func ConcatArrays(typ *types2.T, left Datum, right Datum) (Datum, error) {
...
func ArrayContains(ctx *EvalContext, haystack *DArray, needles *DArray) (*DBool, error) {
	if !haystack.ParamTyp.Equivalent(needles.ParamTyp) {
		return DBoolFalse, pgerror2.New(pgcode.DatatypeMismatch, "cannot compare arrays with different element types")
	}
...
func (o binOpOverload) lookupImpl(left, right *types2.T) (*BinOp, bool) {
	for _, fn := range o {
		casted := fn.(*BinOp)
...
func getJSONPath(j DJSON, ary DArray) (Datum, error) {
	...
	result, err := json2.FetchPath(j.JSON, path)
	if err != nil {
		return nil, err
	}
	if result == nil {
...
func newCannotMixBitArraySizesError(op string) error {
	return pgerror2.Newf(pgcode.StringDataLengthMismatch,
...
var BinOps = map[BinaryOperator]binOpOverload{
	Bitand: {
		...
		&BinOp{
			LeftType:   types2.VarBit,
			RightType:  types2.VarBit,
			ReturnType: types2.VarBit,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
		...
		&BinOp{
			LeftType:   types2.INet,
			RightType:  types2.INet,
			ReturnType: types2.INet,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
	...
	Plus: {
		...
		&BinOp{
			LeftType:   types2.Float,
			RightType:  types2.Float,
			ReturnType: types2.Float,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
		...
		&BinOp{
			LeftType:   types2.Decimal,
			RightType:  types2.Decimal,
			ReturnType: types2.Decimal,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
		...
		&BinOp{
			LeftType:   types2.Date,
			RightType:  types2.Int,
			ReturnType: types2.Date,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
		...
		&BinOp{
			LeftType:   types2.Date,
			RightType:  types2.Time,
			ReturnType: types2.Timestamp,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
		...
		&BinOp{
			LeftType:   types2.Date,
			RightType:  types2.TimeTZ,
			ReturnType: types2.TimestampTZ,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				leftTime, err := left.(*DDate).ToTime()
				if err != nil {
					return nil, err
		...
		&BinOp{
			LeftType:   types2.Time,
			RightType:  types2.Interval,
			ReturnType: types2.Time,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
	...
	Minus: {
		...
		&BinOp{
			...
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				...
				if !l.IsFinite() || !r.IsFinite() {
					return nil, pgerror2.New(pgcode.DatetimeFieldOverflow, "cannot subtract infinite dates")
				}
				a := l.PGEpochDays()
				b := r.PGEpochDays()
		...
		&BinOp{
			LeftType:   types2.Jsonb,
			RightType:  types2.String,
			ReturnType: types2.Jsonb,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
		...
		&BinOp{
			LeftType:   types2.Jsonb,
			RightType:  types2.MakeArray(types2.String),
			ReturnType: types2.Jsonb,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				j := left.(*DJSON).JSON
	...
	Concat: {
		&BinOp{
			LeftType:   types2.String,
			RightType:  types2.String,
			ReturnType: types2.String,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				return NewDString(string(MustBeDString(left) + MustBeDString(right))), nil
			},
		},
		&BinOp{
			LeftType:   types2.Bytes,
			RightType:  types2.Bytes,
			ReturnType: types2.Bytes,
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
		...
		&BinOp{
			...
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				...
				if rval < 0 || rval >= 64 {
					...
					return nil, pgerror2.Newf(pgcode.InvalidParameterValue, "shift argument out of range")
				}
...
func init() {
	timestampMinusBinOp, _ = BinOps[Minus].lookupImpl(types2.TimestampTZ, types2.TimestampTZ)
}
...
func (op *CmpOp) matchParams(l, r *types2.T) bool {
	return op.params().MatchAt(l, 0) && op.params().MatchAt(r, 1)
}

var cmpOpReturnType = FixedReturnType(types2.Bool)
...
func cmpOpFixups(cmpOps map[ComparisonOperator]cmpOpOverload) map[ComparisonOperator]cmpOpOverload {
	...
	for _, t := range types2.Scalar {
		cmpOps[EQ] = append(cmpOps[EQ], &CmpOp{
			LeftType:  types2.MakeArray(t),
			RightType: types2.MakeArray(t),
			Fn:        cmpOpScalarEQFn,
		})
...
func (o cmpOpOverload) LookupImpl(left, right *types2.T) (*CmpOp, bool) {
	for _, fn := range o {
		casted := fn.(*CmpOp)
		if casted.matchParams(left, right) {
			return casted, true
...
func makeCmpOpOverload(
	fn func(ctx *EvalContext, left, right Datum) (Datum, error), a, b *types2.T, nullableArgs bool,
) *CmpOp {
	return &CmpOp{
		LeftType:     a,
		RightType:    b,
...
var CmpOps = cmpOpFixups(map[ComparisonOperator]cmpOpOverload{
	EQ: {
		...
		makeEqFn(types2.Bool, types2.Bool),
		makeEqFn(types2.Bytes, types2.Bytes),
		makeEqFn(types2.Date, types2.Date),
		makeEqFn(types2.Decimal, types2.Decimal),
		makeEqFn(types2.AnyCollatedString, types2.AnyCollatedString),
		makeEqFn(types2.Float, types2.Float),
		makeEqFn(types2.INet, types2.INet),
		makeEqFn(types2.Int, types2.Int),
		makeEqFn(types2.Interval, types2.Interval),
		makeEqFn(types2.Jsonb, types2.Jsonb),
		makeEqFn(types2.Oid, types2.Oid),
		makeEqFn(types2.String, types2.String),
		makeEqFn(types2.Time, types2.Time),
		makeEqFn(types2.TimeTZ, types2.TimeTZ),
		makeEqFn(types2.Timestamp, types2.Timestamp),
		makeEqFn(types2.TimestampTZ, types2.TimestampTZ),
		makeEqFn(types2.Uuid, types2.Uuid),
		makeEqFn(types2.VarBit, types2.VarBit),

		// Mixed-type comparisons.
		makeEqFn(types2.Date, types2.Timestamp),
		makeEqFn(types2.Date, types2.TimestampTZ),
		makeEqFn(types2.Decimal, types2.Float),
		makeEqFn(types2.Decimal, types2.Int),
		makeEqFn(types2.Float, types2.Decimal),
		makeEqFn(types2.Float, types2.Int),
		makeEqFn(types2.Int, types2.Decimal),
		makeEqFn(types2.Int, types2.Float),
		makeEqFn(types2.Timestamp, types2.Date),
		makeEqFn(types2.Timestamp, types2.TimestampTZ),
		makeEqFn(types2.TimestampTZ, types2.Date),
		makeEqFn(types2.TimestampTZ, types2.Timestamp),
		makeEqFn(types2.Time, types2.TimeTZ),
		makeEqFn(types2.TimeTZ, types2.Time),

		// Tuple comparison.
		&CmpOp{
			LeftType:  types2.AnyTuple,
			RightType: types2.AnyTuple,
			Fn: func(ctx *EvalContext, left Datum, right Datum) (Datum, error) {
	...
	LT: {
		// Single-type comparisons.
		makeLtFn(types2.Bool, types2.Bool),
		makeLtFn(types2.Bytes, types2.Bytes),
		makeLtFn(types2.Date, types2.Date),
		makeLtFn(types2.Decimal, types2.Decimal),
		makeLtFn(types2.AnyCollatedString, types2.AnyCollatedString),
		makeLtFn(types2.Float, types2.Float),
		makeLtFn(types2.INet, types2.INet),
		makeLtFn(types2.Int, types2.Int),
		makeLtFn(types2.Interval, types2.Interval),
		makeLtFn(types2.Oid, types2.Oid),
		makeLtFn(types2.String, types2.String),
		makeLtFn(types2.Time, types2.Time),
		makeLtFn(types2.TimeTZ, types2.TimeTZ),
		makeLtFn(types2.Timestamp, types2.Timestamp),
		makeLtFn(types2.TimestampTZ, types2.TimestampTZ),
		makeLtFn(types2.Uuid, types2.Uuid),
		makeLtFn(types2.VarBit, types2.VarBit),

		// Mixed-type comparisons.
		makeLtFn(types2.Date, types2.Timestamp),
		makeLtFn(types2.Date, types2.TimestampTZ),
		makeLtFn(types2.Decimal, types2.Float),
		makeLtFn(types2.Decimal, types2.Int),
		makeLtFn(types2.Float, types2.Decimal),
		makeLtFn(types2.Float, types2.Int),
		makeLtFn(types2.Int, types2.Decimal),
		makeLtFn(types2.Int, types2.Float),
		makeLtFn(types2.Timestamp, types2.Date),
		makeLtFn(types2.Timestamp, types2.TimestampTZ),
		makeLtFn(types2.TimestampTZ, types2.Date),
		makeLtFn(types2.TimestampTZ, types2.Timestamp),
		makeLtFn(types2.Time, types2.TimeTZ),
		makeLtFn(types2.TimeTZ, types2.Time),
	...
	LE: {
		// Single-type comparisons.
		makeLeFn(types2.Bool, types2.Bool),
		makeLeFn(types2.Bytes, types2.Bytes),
		makeLeFn(types2.Date, types2.Date),
		makeLeFn(types2.Decimal, types2.Decimal),
		makeLeFn(types2.AnyCollatedString, types2.AnyCollatedString),
		makeLeFn(types2.Float, types2.Float),
		makeLeFn(types2.INet, types2.INet),
		makeLeFn(types2.Int, types2.Int),
		makeLeFn(types2.Interval, types2.Interval),
		makeLeFn(types2.Oid, types2.Oid),
		makeLeFn(types2.String, types2.String),
		makeLeFn(types2.Time, types2.Time),
		makeLeFn(types2.TimeTZ, types2.TimeTZ),
		makeLeFn(types2.Timestamp, types2.Timestamp),
		makeLeFn(types2.TimestampTZ, types2.TimestampTZ),
		makeLeFn(types2.Uuid, types2.Uuid),
		makeLeFn(types2.VarBit, types2.VarBit),

		// Mixed-type comparisons.
		makeLeFn(types2.Date, types2.Timestamp),
		makeLeFn(types2.Date, types2.TimestampTZ),
		makeLeFn(types2.Decimal, types2.Float),
		makeLeFn(types2.Decimal, types2.Int),
		makeLeFn(types2.Float, types2.Decimal),
		makeLeFn(types2.Float, types2.Int),
		makeLeFn(types2.Int, types2.Decimal),
		makeLeFn(types2.Int, types2.Float),
		makeLeFn(types2.Timestamp, types2.Date),
		makeLeFn(types2.Timestamp, types2.TimestampTZ),
		makeLeFn(types2.TimestampTZ, types2.Date),
		makeLeFn(types2.TimestampTZ, types2.Timestamp),
		makeLeFn(types2.Time, types2.TimeTZ),
		makeLeFn(types2.TimeTZ, types2.Time),
	...
	IsNotDistinctFrom: {
		&CmpOp{
			LeftType:     types2.Unknown,
			RightType:    types2.Unknown,
			Fn:           cmpOpScalarIsFn,
			NullableArgs: true,
		...
		// Single-type comparisons.
		makeIsFn(types2.Bool, types2.Bool),
		makeIsFn(types2.Bytes, types2.Bytes),
		makeIsFn(types2.Date, types2.Date),
		makeIsFn(types2.Decimal, types2.Decimal),
		makeIsFn(types2.AnyCollatedString, types2.AnyCollatedString),
		makeIsFn(types2.Float, types2.Float),
		makeIsFn(types2.INet, types2.INet),
		makeIsFn(types2.Int, types2.Int),
		makeIsFn(types2.Interval, types2.Interval),
		makeIsFn(types2.Jsonb, types2.Jsonb),
		makeIsFn(types2.Oid, types2.Oid),
		makeIsFn(types2.String, types2.String),
		makeIsFn(types2.Time, types2.Time),
		makeIsFn(types2.TimeTZ, types2.TimeTZ),
		makeIsFn(types2.Timestamp, types2.Timestamp),
		makeIsFn(types2.TimestampTZ, types2.TimestampTZ),
		makeIsFn(types2.Uuid, types2.Uuid),
		makeIsFn(types2.VarBit, types2.VarBit),

		// Mixed-type comparisons.
		makeIsFn(types2.Date, types2.Timestamp),
		makeIsFn(types2.Date, types2.TimestampTZ),
		makeIsFn(types2.Decimal, types2.Float),
		makeIsFn(types2.Decimal, types2.Int),
		makeIsFn(types2.Float, types2.Decimal),
		makeIsFn(types2.Float, types2.Int),
		makeIsFn(types2.Int, types2.Decimal),
		makeIsFn(types2.Int, types2.Float),
		makeIsFn(types2.Timestamp, types2.Date),
		makeIsFn(types2.Timestamp, types2.TimestampTZ),
		makeIsFn(types2.TimestampTZ, types2.Date),
		makeIsFn(types2.TimestampTZ, types2.Timestamp),
		makeIsFn(types2.Time, types2.TimeTZ),
		makeIsFn(types2.TimeTZ, types2.Time),
	...
	In: {
		makeEvalTupleIn(types2.Bool),
		makeEvalTupleIn(types2.Bytes),
		makeEvalTupleIn(types2.Date),
		makeEvalTupleIn(types2.Decimal),
		makeEvalTupleIn(types2.AnyCollatedString),
		makeEvalTupleIn(types2.AnyTuple),
		makeEvalTupleIn(types2.Float),
		makeEvalTupleIn(types2.INet),
		makeEvalTupleIn(types2.Int),
		makeEvalTupleIn(types2.Interval),
		makeEvalTupleIn(types2.Jsonb),
		makeEvalTupleIn(types2.Oid),
		makeEvalTupleIn(types2.String),
		makeEvalTupleIn(types2.Time),
		makeEvalTupleIn(types2.TimeTZ),
		makeEvalTupleIn(types2.Timestamp),
		makeEvalTupleIn(types2.TimestampTZ),
		makeEvalTupleIn(types2.Uuid),
		makeEvalTupleIn(types2.VarBit),
	},
	...
	Contains: {
		&CmpOp{
			LeftType:  types2.AnyArray,
			RightType: types2.AnyArray,
			Fn: func(ctx *EvalContext, left Datum, right Datum) (Datum, error) {
		...
		&CmpOp{
			...
			RightType: types2.Jsonb,
			Fn: func(ctx *EvalContext, left Datum, right Datum) (Datum, error) {
				c, err := json2.Contains(left.(*DJSON).JSON, right.(*DJSON).JSON)
				if err != nil {
					return nil, err
				}
	...
	ContainedBy: {
		...
		&CmpOp{
			...
			RightType: types2.Jsonb,
			Fn: func(ctx *EvalContext, left Datum, right Datum) (Datum, error) {
				c, err := json2.Contains(right.(*DJSON).JSON, left.(*DJSON).JSON)
				if err != nil {
					return nil, err
				}
				return MakeDBool(DBool(c)), nil
		...
		&CmpOp{
			...
			Fn: func(ctx *EvalContext, left Datum, right Datum) (Datum, error) {
				...
				if !array.ParamTyp.Equivalent(other.ParamTyp) {
					return nil, pgerror2.New(pgcode.DatatypeMismatch, "cannot compare arrays with different element types")
				}
...
func makeEvalTupleIn(typ *types2.T) *CmpOp {
	return &CmpOp{
		LeftType:  typ,
		RightType: types2.AnyTuple,
...
type EvalPlanner interface {
	...
	ParseType(sql string) (*types2.T, error)
...
func TimestampToInexactDTimestamp(ts hlc.Timestamp) *DTimestamp {
	return MakeDTimestamp(timeutil2.Unix(0, ts.WallTime), time.Microsecond)
}
...
//		return ret, nil
//	}
func queryOid(ctx *EvalContext, typ *types2.T, d Datum) (*DOid, error) {
...
// types.T.
func PerformCast(ctx *EvalContext, d Datum, t *types2.T) (Datum, error) {
	switch t.Family() {
	...
	case types2.StringFamily, types2.CollatedStringFamily:
		var s string
		switch t := d.(type) {
		case *DBitArray:
			s = t.BitArray.String()
		...
		case types2.CollatedStringFamily:
	...
	case types2.TimestampFamily:
		...
		case *DInt:
			return MakeDTimestamp(timeutil2.Unix(int64(*d), 0), roundTo), nil
		case *DTimestamp:
			return d.Round(roundTo), nil
		case *DTimestampTZ:
	...
	case types2.TimestampTZFamily:
		...
		case *DInt:
			return MakeDTimestampTZ(timeutil2.Unix(int64(*d), 0), roundTo), nil
		case *DTimestampTZ:
			return d.Round(roundTo), nil
		}
	...
	case types2.OidFamily:
		...
		case *DOid:
			...
			case oid.T_regtype:
				...
				typ, ok := types2.OidToType[oid.Oid(v.DInt)]
...
func (expr *CollateExpr) Eval(ctx *EvalContext) (Datum, error) {
	...
	default:
		return nil, pgerror2.Newf(pgcode.DatatypeMismatch, "incompatible type for COLLATE: %s", d)
	}
}
...
func ensureExpectedType(exp *types2.T, d Datum) error {
	if !(exp.Family() == types2.AnyFamily || d.ResolvedType().Family() == types2.UnknownFamily ||
...
func (expr *IfErrExpr) Eval(ctx *EvalContext) (Datum, error) {
	...
	if expr.ErrCode != nil {
		...
		if code := pgerror2.GetPGCode(evalErr); code != errpatStr {
			return nil, evalErr
		}
	}
	if expr.Else == nil {
...
func makeNoValueProvidedForPlaceholderErr(pIdx PlaceholderIdx) error {
	return pgerror2.Newf(pgcode.UndefinedParameter,
...
func optimizedLikeFunc(
	...
	case 1:
		...
		case '%':
			if escape == '%' {
				return nil, pgerror2.Newf(pgcode.InvalidEscapeSequence, "LIKE pattern must not end with escape character")
			}
			return func(s string) (bool, error) {
				return true, nil
			}, nil
...
func unescapePattern(
	...
	for i := 0; i < nEscapes; i++ {
		...
		if nextIdx == len(pattern)-len(escapeToken) && emitEscapeCharacterLastError {
			return "", pgerror2.Newf(pgcode.InvalidEscapeSequence, `LIKE pattern must not end with escape character`)
		}
	...
	for i := 0; i < sLen; {
		...
		if sRune == escape {
			...
			if i+w < sLen {
				...
				return false, 0, pgerror2.Newf(pgcode.InvalidEscapeSequence, "LIKE pattern must not end with escape character")
			}
		} else if s[i] == '\\' {
			// We encountered a backslash, so we need to look ahead to figure out how
			// to process it.
			if i+1 == sLen {
				...
				return false, 0, pgerror2.Newf(pgcode.InvalidEscapeSequence, "Unexpected behavior during processing custom escape character.")
			} else if s[i+1] == '\\' {
