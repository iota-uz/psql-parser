...
func DecimalToHLC(d *apd.Decimal) (hlc.Timestamp, error) {
	...
	if err != nil {
		return hlc.Timestamp{}, pgerror2.Wrapf(err, pgcode.Syntax, "parsing argument")
	}
	var logical int64
	if len(parts) > 1 {
		...
		if lp := len(p); lp > logicalLength {
			return hlc.Timestamp{}, pgerror2.Newf(pgcode.Syntax, "logical part has too many digits")
