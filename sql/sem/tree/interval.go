...
func sqlStdToDuration(s string, itm types2.IntervalTypeMetadata) (duration.Duration, error) {
	...
	for i := len(parts) - 1; i >= 0; i-- {
		...
		if strings.ContainsRune(part, ':') {
			...
			if parsedIdx == nothingParsed {
				...
				case types2.IntervalDurationType_MILLISECOND:
...
func parseDuration(s string, itm types2.IntervalTypeMetadata) (duration.Duration, error) {
	var d duration.Duration
...
func (l *intervalLexer) parseShortDuration(
	h int64, hasSign bool, itm types2.IntervalTypeMetadata,
