...
import (
	"fmt"
	lex2 "github.com/iota-uz/psql-parser/sql/lex"
...
func (s *scanner) scanIdent(lval *sqlSymType) {
	...
	if isLower {
		...
		lval.str = lex2.NormalizeName(s.in[start:s.pos])
	}
	...
	lval.id = lex2.GetKeywordID(kw)
	if lval.id != lex2.IDENT {
		if isExperimental {
			if _, ok := lex2.AllowedExperimental[kw]; !ok {
				...
				lval.id = lex2.GetKeywordID(lval.str)
			} else {
...
func (s *scanner) scanNumber(lval *sqlSymType, ch int) {
	...
	for {
		ch := s.peek()
		if (isHex && lex2.IsHexDigit(ch)) || lex2.IsDigit(ch) {
			s.pos++
			continue
		}
		if ch == 'x' || ch == 'X' {
	...
	if hasDecimal || hasExponent {
		...
		lval.union.val = tree2.NewNumVal(floatConst, lval.str, false /* negative */)
	} else {
		...
		lval.union.val = tree2.NewNumVal(intConst, lval.str, false /* negative */)
	}
}

func (s *scanner) scanPlaceholder(lval *sqlSymType) {
	start := s.pos
	for lex2.IsDigit(s.peek()) {
		s.pos++
	}
	lval.str = s.in[start:s.pos]

	placeholder, err := tree2.NewPlaceholder(lval.str)
	if err != nil {
		lval.id = ERROR
		lval.str = err.Error()
		return
...
outer:
	for {
		...
		default:
			...
			if !foundStartTag && !lex2.IsIdentStart(ch) {
				return false
			}
			s.pos++
			if possibleEndTag {
