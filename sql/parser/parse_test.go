...
import (
	...
	"github.com/iota-uz/psql-parser/testutils"
	"github.com/iota-uz/psql-parser/testutils/sqlutils"
	"go/constant"
	"reflect"
	"regexp"
	"strings"
	...
	//_ "github.com/iota-uz/psql-parser/pkg/util/log" // for flags
...
$function$`,
	...
	for _, d := range testData {
		t.Run(d.sql, func(t *testing.T) {
			...
			s := stmts.StringWithFlags(tree2.FmtShowPasswords)
			if d.expected != s {
...
func TestParseTree(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.sql, func(t *testing.T) {
			...
			s := stmts.StringWithFlags(tree2.FmtAlwaysGroupExprs)
			if d.expected != s {
				t.Errorf("%s: expected %s, but found (%d statements): %s", d.sql, d.expected, len(stmts), s)
			}
			if _, err := parser.Parse(s); err != nil {
...
func TestParsePrecedence(t *testing.T) {
	...
	unary := func(op tree2.UnaryOperator, expr tree2.Expr) tree2.Expr {
		return &tree2.UnaryExpr{Operator: op, Expr: expr}
	}
	binary := func(op tree2.BinaryOperator, left, right tree2.Expr) tree2.Expr {
		return &tree2.BinaryExpr{Operator: op, Left: left, Right: right}
	}
	cmp := func(op tree2.ComparisonOperator, left, right tree2.Expr) tree2.Expr {
		return &tree2.ComparisonExpr{Operator: op, Left: left, Right: right}
	}
	not := func(expr tree2.Expr) tree2.Expr {
		return &tree2.NotExpr{Expr: expr}
	}
	and := func(left, right tree2.Expr) tree2.Expr {
		return &tree2.AndExpr{Left: left, Right: right}
	}
	or := func(left, right tree2.Expr) tree2.Expr {
		return &tree2.OrExpr{Left: left, Right: right}
	}
	concat := func(left, right tree2.Expr) tree2.Expr {
		return &tree2.BinaryExpr{Operator: tree2.Concat, Left: left, Right: right}
	}
	regmatch := func(left, right tree2.Expr) tree2.Expr {
		return &tree2.ComparisonExpr{Operator: tree2.RegMatch, Left: left, Right: right}
	}
	regimatch := func(left, right tree2.Expr) tree2.Expr {
		return &tree2.ComparisonExpr{Operator: tree2.RegIMatch, Left: left, Right: right}
	}

	one := tree2.NewNumVal(constant.MakeInt64(1), "1", false /* negative */)
	minusone := tree2.NewNumVal(constant.MakeInt64(1), "1", true /* negative */)
	two := tree2.NewNumVal(constant.MakeInt64(2), "2", false /* negative */)
	minustwo := tree2.NewNumVal(constant.MakeInt64(2), "2", true /* negative */)
	three := tree2.NewNumVal(constant.MakeInt64(3), "3", false /* negative */)
	a := tree2.NewStrVal("a")
	b := tree2.NewStrVal("b")
	c := tree2.NewStrVal("c")
	...
	testData := []struct {
		...
		{`~-1`, unary(tree2.UnaryComplement, minusone)},
		{`-~1`, unary(tree2.UnaryMinus, unary(tree2.UnaryComplement, one))},

		// Mul, div, floordiv, mod combined with higher precedence.
		{`-1*2`, binary(tree2.Mult, minusone, two)},
		{`1*-2`, binary(tree2.Mult, one, minustwo)},
		{`-1/2`, binary(tree2.Div, minusone, two)},
		{`1/-2`, binary(tree2.Div, one, minustwo)},
		{`-1//2`, binary(tree2.FloorDiv, minusone, two)},
		{`1//-2`, binary(tree2.FloorDiv, one, minustwo)},
		{`-1%2`, binary(tree2.Mod, minusone, two)},
		{`1%-2`, binary(tree2.Mod, one, minustwo)},

		// Mul, div, floordiv, mod combined with self (left associative).
		{`1*2*3`, binary(tree2.Mult, binary(tree2.Mult, one, two), three)},
		{`1*2/3`, binary(tree2.Div, binary(tree2.Mult, one, two), three)},
		{`1/2*3`, binary(tree2.Mult, binary(tree2.Div, one, two), three)},
		{`1*2//3`, binary(tree2.FloorDiv, binary(tree2.Mult, one, two), three)},
		{`1//2*3`, binary(tree2.Mult, binary(tree2.FloorDiv, one, two), three)},
		{`1*2%3`, binary(tree2.Mod, binary(tree2.Mult, one, two), three)},
		{`1%2*3`, binary(tree2.Mult, binary(tree2.Mod, one, two), three)},
		{`1/2/3`, binary(tree2.Div, binary(tree2.Div, one, two), three)},
		{`1/2//3`, binary(tree2.FloorDiv, binary(tree2.Div, one, two), three)},
		{`1//2/3`, binary(tree2.Div, binary(tree2.FloorDiv, one, two), three)},
		{`1/2%3`, binary(tree2.Mod, binary(tree2.Div, one, two), three)},
		{`1%2/3`, binary(tree2.Div, binary(tree2.Mod, one, two), three)},
		{`1//2//3`, binary(tree2.FloorDiv, binary(tree2.FloorDiv, one, two), three)},
		{`1//2%3`, binary(tree2.Mod, binary(tree2.FloorDiv, one, two), three)},
		{`1%2//3`, binary(tree2.FloorDiv, binary(tree2.Mod, one, two), three)},
		{`1%2%3`, binary(tree2.Mod, binary(tree2.Mod, one, two), three)},

		// Binary plus and minus combined with higher precedence.
		{`1*2+3`, binary(tree2.Plus, binary(tree2.Mult, one, two), three)},
		{`1+2*3`, binary(tree2.Plus, one, binary(tree2.Mult, two, three))},
		{`1*2-3`, binary(tree2.Minus, binary(tree2.Mult, one, two), three)},
		{`1-2*3`, binary(tree2.Minus, one, binary(tree2.Mult, two, three))},

		// Binary plus and minus combined with self (left associative).
		{`1+2-3`, binary(tree2.Minus, binary(tree2.Plus, one, two), three)},
		{`1-2+3`, binary(tree2.Plus, binary(tree2.Minus, one, two), three)},

		// Left and right shift combined with higher precedence.
		{`1<<2+3`, binary(tree2.LShift, one, binary(tree2.Plus, two, three))},
		{`1+2<<3`, binary(tree2.LShift, binary(tree2.Plus, one, two), three)},
		{`1>>2+3`, binary(tree2.RShift, one, binary(tree2.Plus, two, three))},
		{`1+2>>3`, binary(tree2.RShift, binary(tree2.Plus, one, two), three)},

		// Left and right shift combined with self (left associative).
		{`1<<2<<3`, binary(tree2.LShift, binary(tree2.LShift, one, two), three)},
		{`1<<2>>3`, binary(tree2.RShift, binary(tree2.LShift, one, two), three)},
		{`1>>2<<3`, binary(tree2.LShift, binary(tree2.RShift, one, two), three)},
		{`1>>2>>3`, binary(tree2.RShift, binary(tree2.RShift, one, two), three)},

		// Power combined with lower precedence.
		{`1*2^3`, binary(tree2.Mult, one, binary(tree2.Pow, two, three))},
		{`1^2*3`, binary(tree2.Mult, binary(tree2.Pow, one, two), three)},

		// Bit-and combined with higher precedence.
		{`1&2<<3`, binary(tree2.Bitand, one, binary(tree2.LShift, two, three))},
		{`1<<2&3`, binary(tree2.Bitand, binary(tree2.LShift, one, two), three)},

		// Bit-and combined with self (left associative)
		{`1&2&3`, binary(tree2.Bitand, binary(tree2.Bitand, one, two), three)},

		// Bit-xor combined with higher precedence.
		{`1#2&3`, binary(tree2.Bitxor, one, binary(tree2.Bitand, two, three))},
		{`1&2#3`, binary(tree2.Bitxor, binary(tree2.Bitand, one, two), three)},

		// Bit-xor combined with self (left associative)
		{`1#2#3`, binary(tree2.Bitxor, binary(tree2.Bitxor, one, two), three)},

		// Bit-or combined with higher precedence.
		{`1|2#3`, binary(tree2.Bitor, one, binary(tree2.Bitxor, two, three))},
		{`1#2|3`, binary(tree2.Bitor, binary(tree2.Bitxor, one, two), three)},

		// Bit-or combined with self (left associative)
		{`1|2|3`, binary(tree2.Bitor, binary(tree2.Bitor, one, two), three)},

		// Equals, not-equals, greater-than, greater-than equals, less-than and
		// less-than equals combined with higher precedence.
		{`1 = 2|3`, cmp(tree2.EQ, one, binary(tree2.Bitor, two, three))},
		{`1|2 = 3`, cmp(tree2.EQ, binary(tree2.Bitor, one, two), three)},
		{`1 != 2|3`, cmp(tree2.NE, one, binary(tree2.Bitor, two, three))},
		{`1|2 != 3`, cmp(tree2.NE, binary(tree2.Bitor, one, two), three)},
		{`1 > 2|3`, cmp(tree2.GT, one, binary(tree2.Bitor, two, three))},
		{`1|2 > 3`, cmp(tree2.GT, binary(tree2.Bitor, one, two), three)},
		{`1 >= 2|3`, cmp(tree2.GE, one, binary(tree2.Bitor, two, three))},
		{`1|2 >= 3`, cmp(tree2.GE, binary(tree2.Bitor, one, two), three)},
		{`1 < 2|3`, cmp(tree2.LT, one, binary(tree2.Bitor, two, three))},
		{`1|2 < 3`, cmp(tree2.LT, binary(tree2.Bitor, one, two), three)},
		{`1 <= 2|3`, cmp(tree2.LE, one, binary(tree2.Bitor, two, three))},
		{`1|2 <= 3`, cmp(tree2.LE, binary(tree2.Bitor, one, two), three)},

		// NOT combined with higher precedence.
		{`NOT 1 = 2`, not(cmp(tree2.EQ, one, two))},
		{`NOT 1 = NOT 2 = 3`, not(cmp(tree2.EQ, one, not(cmp(tree2.EQ, two, three))))},

		// NOT combined with self.
		{`NOT NOT 1 = 2`, not(not(cmp(tree2.EQ, one, two)))},
		...
		{`~1+2`, binary(tree2.Plus, unary(tree2.UnaryComplement, one), two)},
	}
	for _, d := range testData {
		t.Run(d.sql, func(t *testing.T) {
