...
type Statement struct {
	...
	NumAnnotations tree2.AnnotationIdx
}
...
func (stmts Statements) String() string {
	return stmts.StringWithFlags(tree2.FmtSimple)
}

// StringWithFlags returns the AST formatted as a string (with the given flags).
func (stmts Statements) StringWithFlags(flags tree2.FmtFlags) string {
	ctx := tree2.NewFmtCtx(flags)
	for i, s := range stmts {
		if i > 0 {
			ctx.WriteString("; ")
		}
...
func unaryNegation(e tree2.Expr) tree2.Expr {
	if cst, ok := e.(*tree2.NumVal); ok {
		cst.Negate()
		return cst
	}

	// Common case.
	return &tree2.UnaryExpr{Operator: tree2.UnaryMinus, Expr: e}
}
...
func ParseQualifiedTableName(sql string) (*tree2.TableName, error) {
...
func ParseTableName(sql string) (*tree2.UnresolvedObjectName, error) {
	...
	rename, ok := stmt.AST.(*tree2.RenameTable)
	if !ok {
...
func parseExprs(exprs []string) (tree2.Exprs, error) {
	stmt, err := ParseOne(fmt.Sprintf("SET ROW (%s)", strings.Join(exprs, ",")))
	if err != nil {
		return nil, err
	}
	set, ok := stmt.AST.(*tree2.SetVar)
	if !ok {
...
func ParseExprs(sql []string) (tree2.Exprs, error) {
	if len(sql) == 0 {
		return tree2.Exprs{}, nil
	}
	return parseExprs(sql)
}

// ParseExpr is a short-hand for parseExprs([]string{sql})
func ParseExpr(sql string) (tree2.Expr, error) {
