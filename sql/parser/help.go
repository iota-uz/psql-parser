...
func helpWithFunction(sqllex sqlLexer, f tree2.ResolvableFunctionReference) int {
	...
	for i, overload := range d.Definition {
		...
		simplifyRet := d.Class == tree2.GeneratorClass
...
func helpWithFunctionByName(sqllex sqlLexer, s string) int {
	un := &tree2.UnresolvedName{NumParts: 1, Parts: tree2.NameParts{s}}
	return helpWithFunction(sqllex, tree2.ResolvableFunctionReference{FunctionReference: un})
}
